    ...
    <section className="section-padding bg-foreground">
      <div className="container max-w-5xl">
        <div className="text-center mb-12 space-y-4">
          <h2 className="text-3xl sm:text-4xl lg:text-5xl font-playfair font-bold tracking-tight text-background leading-tight">
            Your customers hesitate.
            <br />
            And hesitation <span className="text-gradient italic">kills revenue.</span>
          </h2>
          <p className="font-inter text-background/60 text-lg max-w-2xl mx-auto">
        ...
        <div className="grid sm:grid-cols-3 gap-6">
          {painPoints.map((point, index) => (
            <div
              key={index}
              className="bg-background/10 border border-background/10 rounded-2xl p-8 text-center space-y-4"
            >
              ...
              <p className="text-sm font-inter text-background/50 leading-relaxed">{point.description}</p>
            </div>
          ))}
        </div>
      </div>
